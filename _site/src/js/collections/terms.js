/******************************************************************************\
|                                                                              |
|                                  terms.js                                    |
|                                                                              |
|******************************************************************************|
|                                                                              |
|        This is a collection of terms search results.                         |
|                                                                              |
|        Author(s): Abe Megahed                                                |
|                                                                              |
|        This file is subject to the terms and conditions defined in           |
|        'LICENSE.txt', which is part of this source code distribution.        |
|                                                                              |
|******************************************************************************|
|        Copyright (C) 2012-2020 Morgridge Institute for Research (MIR)        |
\******************************************************************************/

define([
	'collections/results'
], function(Results) {
	'use strict';
	
	return Results.extend({

		//
		// attributes
		//

		baseUrl: Results.prototype.baseUrl + '/terms',
		
		//
		// searching methods
		//

		search: function(options) {
			var params = {};

			// set API params from options
			//
			if (options.data) {

				// set dataset param
				//
				if (options.data.dataset) {
					params.dataset = options.data.dataset;
				}
				
				// set term param
				//
				if (options.data.term) {
					params.term = options.data.term;
				}

				// set dictionary param
				//
				if (options.data.dictionary) {
					params.dictionary = options.data.dictionary;
				}

				// set publishing param
				//
				if (options.data.publisher) {
					params.publisher = options.data.publisher;
				}
			}
			
			// perform search
			//
			this.fetch({
				data: params,
				success: options.success,
				error: options.error
			});
		},
	});
});
